...
class DoraConfig(PeftConfig):
    ...
    # Added for layer-specific targeting
    layer_specific_target_modules: Optional[dict] = field(
        default=None,
        metadata={
            "help": "Dictionary mapping layer numbers to target module names. "
                    "Example: {'01': ['k_proj', 'v_proj'], '02': ['q_proj']}"
        },
    )
    ...
    # Added for layer-specific Wdecompose targeting
    layer_specific_Wdecompose_modules: Optional[dict] = field(
        default=None,
        metadata={
            "help": "Dictionary mapping layer numbers to Wdecompose module names. "
                    "Example: {'01': ['k_proj', 'v_proj'], '02': ['q_proj']}"
        },
    )
...
class DoraModel(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def _is_target_module(self, key):
        ...
        if self.peft_config.layer_specific_target_modules is not None:
            ...
            if layer_match:
                ...
                for specific_layer in self.peft_config.layer_specific_target_modules:
    ...
    def _is_Wdecompose_target_module(self, key):
        """Check if a module should be targeted for Wdecompose based on both general and layer-specific rules."""
        # Added Layer-Specific Wdecompose Target Checking
        if self.peft_config.layer_specific_Wdecompose_modules is not None:
            layer_match = re.search(r'\.(\d+)\.', key)
            if layer_match:
                layer_num = layer_match.group(1)
                if layer_num in self.peft_config.layer_specific_Wdecompose_modules:
                    return any(target in key for target in self.peft_config.layer_specific_Wdecompose_modules[layer_num])

        # General Wdecompose target module check
        if self.peft_config.Wdecompose_target_modules is None:
            return False
        if isinstance(self.peft_config.Wdecompose_target_modules, str):
            return bool(re.fullmatch(self.peft_config.Wdecompose_target_modules, key))
        return any(key.endswith(target_key) for target_key in self.peft_config.Wdecompose_target_modules)
    ...
    def _find_and_replace(self):
        ...
        for key in key_list:
            ...
            elif self._is_Wdecompose_target_module(key):
                if not is_target_modules_in_base_model:
                    is_target_modules_in_base_model = True
                parent, target, target_name = self._get_submodules(key)
                bias = target.bias is not None
        ...
        # Ensure layer-specific targets are checked
        if not is_target_modules_in_base_model and (
            self.peft_config.layer_specific_target_modules or self.peft_config.layer_specific_Wdecompose_modules
        ):
            raise ValueError(
                "Layer-specific target modules not found in the base model. "
                "Please check the layer-specific configuration and try again."
            )
